...
namespace POS.frm
{
    public partial class Customer : Form
    {
        ...
        public Customer(MenuUtama mnutama)
        {
            InitializeComponent();
        ...
        private void UsrAccess()
        {
            string data = qry.GetData(this, "UsrAccess", userName, 0,
                "pelanggansv,pelanggandel,pelangganprt,pelangganup", "usrmgmt", "usrname = '" + userName + "'");
        ...
        private void SelectToolBar(Button btn)
        {
            switch (btn.AccessibleName)
            {
                ...
                case "HAPUS":
                    ...
                    {
                        psn.PesanInfo("ID Customer Masih Kosong");
                    }
                    else
                    {
                        Hapus();
        ...
        private void TampilData(string id)
        {
            ...
            string data = qry.GetData(this, "TampilData", id, 8, "id,cekawal,nama,alamat,telp,nohp,fax,email,npwp,website,catatan", "customer", "id = '" + id + "'");
            ...
            if (data != "")
            {
                ...
                if (filldata.ToString() != "")
                {
                    ...
                    chkCustomerAwal.Checked = fs.IntToBool(filldata[1]);
                    txtNama.Text = filldata[2];
                    txtAlamat.Text = filldata[3];
                    txtNoTelp.Text = filldata[4];
                    txtNoHP.Text = filldata[5];
                    txtFax.Text = filldata[6];
                    txtEmail.Text = filldata[7];
                    txtNPWP.Text = filldata[8];
                    txtWebsite.Text = filldata[9];
                    txtCatatan.Text = filldata[10];
        ...
        private void Simpan()
        {
            if (qry.DataExist(this, "Simpan", lblID.Text, txtNama.Text.Trim(), "id", "customer", "id = '" + lblID.Text + "'"))
            {
                try
                {
                    if(chkCustomerAwal.Checked == true)
                    {
                        string idcustlama = "";
                        string namacustlama = "";

                        string data = qry.GetData(this, "Simpan-Cek Cust Awal Lama", "", 8, "id,nama", "customer", "cekawal = 1");
                        ...
                        if (data != "")
                        {
                            ...
                            if (filldata.ToString() != "")
                            {
                                idcustlama = filldata[0];
                                namacustlama = filldata[1];

                                qry.InsertLogAktivitas("Simpan-Cek Cust Awal Lama", this, idcustlama, userName);
                            }
                            qry.UpdateData("customer",
                            "cekawal = 0",
                            "id = '" + idcustlama + "'");

                            qry.InsertLogAktivitas("Update-Cek Cust Awal Lama", this, idcustlama, userName);
                            fs.ToolTipTransaksi(frmUtama.toolTip2, frmUtama, "Update-Cek Cust Awal Lama " + this.Name.ToString() + " " + idcustlama + " Berhasil",
                                frmUtama.tooltip_x, frmUtama.tooltip_y, "info");
                        }
                    }

                    qry.UpdateData("customer",
                        "cekawal = " + fs.BoolToInt(chkCustomerAwal.Checked) + "," +
            ...
            else
            {
                if (lblID.Text == "AUTO ID")
                {
                    lblID.Text = qry.CreateID(this, "Simpan", "id", "customer", "id like 'CS.%'", "CS.", "master");
                }
                ...
                try
                {
                    ...
                    qry.InsertData("customer",
                        "id,cekawal,nama,alamat,telp,nohp,fax,email,npwp,website,catatan,tglinput,tglupdate",
                        "'" + lblID.Text + "'," + fs.BoolToInt(chkCustomerAwal.Checked) + ",'" + txtNama.Text.Trim() + "','" + txtAlamat.Text.Trim() + "'," +
        ...
        private void Hapus()
        {
            ...
            {*/
                ...
                if (result == DialogResult.Yes)
                {
                    try
                    {
                        qry.DeleteData("customer", "id = '" + lblID.Text + "'");
        ...
        private void Customer_Load(object sender, EventArgs e)
        {
            ...
            frmUtama.lblNamaForm.Text = "- Customer";
            UsrAccess();
        }
